...
def uploadIpfs(fileKey, author, fileName, localBlockchain, fileContent, FILENAME, targetIP, accessList, fileLocation):
    try:
        ...
        fernet = Fernet('yrnX6PYjWcITai_1Ux6IC1rXlCP7y1TiPC8dcxTi7os=')#(base64.urlsafe_b64encode(fileKey))
...
def getFileList(localBlockchain):#, role, username):
    ...
    #Else, role is patient
        ...
        #    if block.index == 0:
        #        continue
            #If authorized to view the file, view the file
        #    if block.transactionType == "Upload" and block.payload.fileName not in newFileNameList and username in block.payload.accessList:
        #        newFileNameList.append(block.payload.fileName)
...
def getUserRole(localBlockchain, username):
    currentData = "####"
    i = 0#: len(localBlockchain)
    #Loops through the entire blockchain given
    while i < len(localBlockchain):# block in localBlockchain:
        #If a block for an account
        if localBlockchain[i].transactionType == "Create_Account":
            #If the username of the block matches that of 
            if localBlockchain[i].payload.username == username:
                currentData = localBlockchain[i].payload.role
                break
        i = i - 1
    #Thus, account exists
    if currentData != "####":
        j = len(localBlockchain) - 1
        while j >= 0:
            if localBlockchain[j].transactionType == "Update_User":
                #If the username of the block matches that of 
                if localBlockchain[j].payload.username == username:
                    return localBlockchain[j].payload.role
            j = j - 1
        #Else, not found, so return the current data 
        return currentData
    else:
        return None
"""
...
def sendRequest(sendRequestMessage, blockToAdd, targetIP="146.229.163.145", authorizingUser=None):
    try:
        ...
        if sendRequestMessage == "Upload_File":
            ...
            clientSocket.send((sendRequestMessage).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((blockToAdd.ipfsHash).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((blockToAdd.fileName).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((blockToAdd.authorName).encode("utf-8")[:8192])
            for item in blockToAdd.accessList:
                clientSocket.send((" ").encode("utf-8")[:8192])
                clientSocket.send((str(item)).encode("utf-8")[:8192])
            #Closes the socket
            clientSocket.close()
            ...
            clientSocket.send((sendRequestMessage).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((authorizingUser).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((blockToAdd.ipfsHash).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((blockToAdd.fileName).encode("utf-8")[:8192])
            ...
            time.sleep(0.5)
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((str(authorizingUser)).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((str(blockToAdd.username)).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((str(blockToAdd.password)).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((str(blockToAdd.role)).encode("utf-8")[:8192])
            clientSocket.send((" ").encode("utf-8")[:8192])
            clientSocket.send((str(blockToAdd.fullLegalName)).encode("utf-8")[:8192])
            #Closes the socket
            clientSocket.close()
            #If this line is reached(no error have resulted), returns the status of the operation as true
            return "True"
        #Code to get the encryption key
            ...
            clientSocket.send((blockToAdd.save_pkcs1().decode("utf-8")).encode("utf-8")[:8192])
            #Waits for reception of the file key and decrypts it with the privateKey
            return rsa.decrypt(clientSocket.recv(8192), authorizingUser)
        #If the desire is to get the blockchain, send a request for it
            ...
            NewBlockchain = clientSocket.recv(8192).decode("utf-8")
            #Closes the socket
            clientSocket.close()
